...
`max_concurrent_market_evaluations` bounds scan-all work. Polymarket, Predict.fun, Gamma, and Myriad bootstrap HTTP traffic is semaphore-limited and all clients reuse long-lived `aiohttp` sessions. Set `shadow_mode=true` to exercise discovery, books, matching, sizing, and alerts with order submission and production balance gates disabled.
