...
candidates not observed within `discovery_max_stale_seconds` are excluded. The
approval clock uses the dedicated `last_discovered_at` evidence field; ordinary
runtime row updates and status changes cannot refresh it.
...
`max_concurrent_market_evaluations` bounds the active market-data window, while `max_concurrent_market_evaluations_by_route` can impose a lower hard cap on a venue-sensitive route without shrinking the full discovery universe. The production quote service keeps 12 `polymarket_myriad` evaluations active and prefetches exactly that same window for 20 seconds. This retains the full-universe turnover of the former 36-target/60-second window without rotating three funded subsets on every poll. The Myriad connector reserves 12 request slots for exact funded refreshes plus four for discovery and paces funded REST starts by 50 ms after capacity is reserved. Its global latest-safe-start schedule gives synchronized or compressed receipts distinct deadline-ordered slots, while already separated receipts retain the normal 0.85-second trigger instead of sustaining an early refresh loop. The original book's absolute freshness deadline bounds queueing and request dispatch; once dispatched, the request uses its separate bounded HTTP timeout while entry readiness remains fail-closed if the old book ages out. A newer WebSocket/bootstrap receipt cancels the redundant GET, while an already-stale target gets one separately bounded recovery attempt and remains fail-closed until it succeeds. Telemetry separates actual requests, dispatch-deadline misses, no-receipt completions, coalesced waiters, and receipt-advancing refreshes. The engine rotates each window across the full eligible universe after `market_data_target_hold_seconds`, allowing WebSocket snapshots to warm without increasing concurrency. Polymarket, Predict.fun, and Myriad bootstrap HTTP traffic is bounded and all clients reuse long-lived `aiohttp` sessions. Polymarket discovery uses sequential 1,000-market CLOB pages plus Gamma ID batches of up to 50, eliminating individual Gamma lookups. Set `shadow_mode=true` to exercise discovery, books, matching, sizing, and alerts with order submission and production balance gates disabled.
